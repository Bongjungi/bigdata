...
SELECT EMPNO AS "사번", ENAME AS "이름", SAL AS "급여",JOB AS "직책" 
    FROM EMP;--별칭 ---자바 사용할 때 1.같은 필드이름이 있거나 2.필드이름에 특수문자가 들어가면 안되기 때문에 별칭으로 고쳐야 한다.
SELECT EMPNO AS 사번, ENAME AS 이름, SAL AS 급여,JOB AS 직책 
    FROM EMP;--별칭
SELECT EMPNO 사번, ENAME 이름, SAL 급여,JOB 직책 
    FROM EMP;--별칭
SELECT EMPNO "사 번", ENAME "이 름", SAL "급 여",JOB "직 책" 
    FROM EMP;--별칭
SELECT EMPNO NO, ENAME NAME, SAL SALARY,JOB JOB 
    FROM EMP;--별칭
