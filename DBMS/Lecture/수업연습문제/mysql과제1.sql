...
select w.pno, w.pname, if(m.pname is null,'상사 없음',m.pname) from personal w left outer join personal m on w.manager=m.pno;
-- 9. 이름이 s로 시작하는 사원 이름
...
select pname, pay, dno, (select avg(pay) from personal where dno=p.dno) avgsal from personal p where pay>(select avg(pay) from personal where dno=p.dno);
...
-- 20. 이름, 급여, 부서명, 해당부서평균
select p.pname, p.pay, s.dname, s.avgpay 
	from (select personal.dno, dname, avg(pay) avgpay from personal, division where personal.dno=division.dno group by dname) s, personal p 
	where s.dno=p.dno;