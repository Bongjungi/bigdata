...
SELECT ROWNUM 등수, DEPTNO, SUMSAL 
    FROM (SELECT DEPTNO, SUM(SAL) SUMSAL 
             FROM EMP WHERE SAL IN (SELECT SAL FROM EMP E WHERE E.DEPTNO=DEPTNO) GROUP BY DEPTNO ORDER BY SUM(SAL) DESC);
...
SELECT 등수, DEPTNO, SUMSAL 
    FROM (SELECT ROWNUM 등수, DEPTNO, SUMSAL  
             FROM (SELECT DEPTNO, SUM(SAL) SUMSAL 
                      FROM EMP WHERE SAL IN (SELECT SAL FROM EMP E WHERE E.DEPTNO=DEPTNO) GROUP BY DEPTNO ORDER BY SUM(SAL) DESC))
...
SELECT EMPNO, ENAME, HIREDATE
    ...
    WHERE RN<6;
SELECT EMPNO, ENAME, HIREDATE
    FROM (SELECT EMPNO, ENAME, HIREDATE FROM EMP ORDER BY HIREDATE DESC) WHERE ROWNUM<6;
