...
#ggplot2::diamonds에서 cut별 빈도수
table(diamonds$cut)
ggplot(diamonds, aes(x=cut, col=cut))+
  geom_bar(stat="count")
?geom_bar
...
x <- diamonds %>% 
  group_by(cut,table) %>% 
  summarise(n=n())
y <- x[x$cut=='Ideal' | x$cut=='Good',]
edit(y)
ggplot(y, aes(x=table, y=n))+
  geom_bar(stat="identity")+
  facet_wrap(~cut) + 
  coord_cartesian(ylim = c(0,6000),xlim=c(50,80))#하나하나하면 괜찮은데
...
diamonds %>% 
  ...
  geom_col()+
  facet_wrap(~cut)+
  coord_cartesian(ylim = c(0,6000),xlim=c(50,80))##단체로 하면 왜 이상하냐.
...
fitted(fit) ## = e값 제외 회귀식에 speed값을 넣을때의 dist값 #fitted value
residuals(fit) ## 잔차
rstandard(fit) # 표준잔차 (잔차를 표준잔차로 나눔) residuals(fit)/sd(residuals(fit))

sqrt(abs(rstandard(fit)))
